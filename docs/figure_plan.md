...
| R4 | **Buffer vs tail** | More buffer stops drops but not starvation; makespan follows the max FCT, on both fabrics. | Two small panels: (a) NVL-64 k-sweep, (b) Glass edge 1600→3200 at EP=64; each: makespan (left axis, bars) + max FCT (right axis, line), RTO count as labels. | buffer_sweeps.csv (built by `build_buffer_sweeps.py`). **Not** edge_ep64.csv: it is pre-fix (2026-09-06, the fix landed 09-07 10:01) and its swept edge rates 1600/2400/3200 GB/s all truncated to zero ps/byte, so the variable it sweeps had no effect on transmission time |
