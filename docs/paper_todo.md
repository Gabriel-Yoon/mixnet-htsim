...
- **E46 [BLOCKED — figures are not yet reproducible]** The figure pipeline lives only on the local machine; nothing on PACE could produce any paper figure. Now committed by the peer session (`55d4b9c`: `FIGURES.md`, 14 plot scripts, 19 figure CSVs). Remaining gaps recorded there: **fig_baselines, fig_phases and fig_isopower have no identified producer** — their source CSVs are known but the script that drew them is not in any repo, so their provenance is inferred from magnitude, not recorded. `plot_isopower.py` hardcodes fat-tree values (245.052 / 160.186 / 118.456 / 91.101) that are an older pass than the published 190 / 139 / 105. And the `COMPUTE = {mixtral8x7B: 102.0, llamaMoE: 75.0, qwenMoE: 70.0}` constants have **no traceable source at all** — they must be re-derived from compute-only runs and carried as a CSV column, never a constant. All three figures get regenerated with a recorded chain.
