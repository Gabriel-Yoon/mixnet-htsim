...
| `fig_copperfb` (Fig 7c) | **uncertain** — candidates: `plot_fabric_breakdown.py` → `mixtral_fabric_breakdown.png` (reads `mixtral_fabric_sweep*.csv`), `plot_medium_ablation_per_topo.py` (peer, newer) | `mixtral_fabric_sweep.csv`, `mixtral_fabric_sweep_v2.csv` | mixtral8x7B dp2tp4pp4 (analytical) | **producer not identified**; regenerate with a recorded chain. |
