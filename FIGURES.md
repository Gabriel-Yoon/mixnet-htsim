...
| `fig_thermal_panel` (left, NEW) | `scripts/figures/plot_thermal_panel.py` → `thermal_panel_steady.png` | `experiments/results/thermal/panel_plane_glass.csv`, `panel_tiles_{glass,si}.csv` (0f757db) ← `panhq_{glass,si}.rth` ← `thermal_panel_body.inp` via `run_panel_hq.sh` (NX=4, PGPU=700, FHOT=0.5, TCP_IN=55, TCP_RISE=20, NDPT=32; **HCP=100000 per the solve's own `.db` (`RESUME` + `*STATUS`), NOT the 70000 the runner script declares**; PIC layer has NO heat source — the field is die-imposed). Glass PIC peak 119.3 °C (outlet column), Si control 115.4, both at h=100k. Re-solve at the Coenen-cited 70k pending. Rule: recover boundary conditions from the solve artifact, never from the script that was supposed to set them. **Replaces** the published left panel, whose fingerprint (tile (0,0) = 94.0) matches neither hq solve and is attributed to `_run_panel_small.sh` (HCP=100000, NDPT=6) — the 94.0 file matches neither hq solve; since panhq itself ran at 100k, the distinguishing variable is the mesh (NDPT=6), not h. Three panel configurations exist on disk; only panhq has recovered parameters. |
| `fig_thermal_transient` (right) | `thermal_pic/plot_pic_transient.py` → `fig_pic_mrr_transient.png` | `thermal_pic/{mixtral8x7b,qwenmoe,llamamoe}_pic_transient_L4seq4096.csv` ← `pic_transient_*_L4seq4096.inp` (`gen_pic_transient.py`, OIO3D stack, Coenen params, FLOP-based GPU power schedule) | committed chain. Excursion ≤9 K peak-to-base (Mixtral/LLaMA), ~4.5 K (Qwen); pulse period ≈0.4 s. The ASPDAC caption's "≤7.8 K, ~13 Hz idle↔TDP" figure has no committed producer (possibly the PACE-only `stab_*` sweep) and is no longer quoted. |
