...
| `fig_thermal_transient` | `scripts/figures/plot_thermal_transient.py` → `fig_thermal_transient.png`: `experiments/results/thermal/tile_schedule.csv` (status final, glass; delta_pp_K vs period_s per p_lo_W) and `tile_transient.csv` glass_stack_h200k_long step asymptote; curves are a first-order model (tau = rise/2.197 from the step) times (P_hi-P_lo)/P_hi times tanh(T/4tau), not a fit -- it reproduces the 86.75 ms ANSYS points within 1% and overestimates the 10.84 ms points by ~1 K; period sweep requested from the peer 2026-09-08 |
