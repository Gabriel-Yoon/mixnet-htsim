...
| R3 | **Configuration waterfall** | Two configuration corrections and one collective change account for the whole gap: mesh → port map → dim-order relay → hierarchical A2A. | Waterfall/bars at EP=32: 186.4 → … with NVL-64 pkt and island bound as horizontal lines; ablation ladder A/B/C folded in as an inset if space. | ladder.csv + attribution cell |
