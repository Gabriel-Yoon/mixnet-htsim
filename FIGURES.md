...
| `fig_load` | `scripts/figures/plot_paper.py load` → `fig_load.png`: EP=16 mb 4/8/16/32 for glass_200G, nvl64_striped and hgx8 from `load_panel.csv` (peer-derived from `cliff_postfix.csv`, quoted rung per point with its drop count; HGX-8 mb 4/16/32 from batch 11) |
| `fig_calib` | `scripts/figures/plot_paper.py calibfig` → `fig_calib.png`: `calib_nvswitch.csv` quoted rung per message size (s1 = this work, s18 = rejected per-lane variant) and `simai_calib.csv` (cef83f9); bands are li-tpds20 (45–85 µs) and DeepEP (71–82%) |
| `tables/energy_table.tex` | `scripts/figures/energy_table.py` → every term of the energy accounting from `power_tiers.csv` (glassfb_800) and `power_tiers_pkt.csv` (nvl64_pkt_s1, hgx8_pkt); bytes columns are hop-bytes as the producers emit them (no second hop factor) |
| `fig_dse` | `scripts/figures/plot_paper.py dse` → `fig_dse.png`: every rung of the 4x4 design point, the 8x8 flattened butterfly (opt_bw 128) and the 8x8 electrical mesh (GLASS_MAXDIST=1) from `panel_dse.csv` (derived by `scripts/build_panel_dse.py` from `cliff_postfix.csv`; batch 10, binary 95c35a7), x = q_over_bdp; points above the panel's range are clipped and labelled. **EP 16/32/64 only.** EP=128 was attempted and INVALIDATED, not left pending: all twelve EP=128 jobs of both arms came back `status=blocked relayed_pairs=1` on uncabled panel pair (12,14), because `p64_ep128.txt` cables 28 panel pairs and the workload uses 36 (missing (0,2) (1,3) (4,6) (5,7) (8,10) (9,11) (12,14) (13,15)). A blocked row is a different fabric, not a slow one. Dropped by user decision rather than re-run; see sub-class Q in `docs/methods_provenance.md`. NOTE for any plotting script: `panel_dse.csv` still carries EP=128 rows, but they are the `arm=4x4` REFERENCE ladder alone -- there is no 8x8 arm at that EP, so an EP=128 panel would be a lone reference line. |
| `fig_panel_size` | `scripts/figures/plot_panel_size.py` → `fig_panel_size.png`: quotable rows of `panel_dse.csv`, iteration of the 8x8 flattened butterfly (128 GB/s optical links) and the 8x8 electrical mesh normalized to the 4x4 panel at EP 16/32/64; the 8x4 arm is not drawn (user decision 2026-09-09). In the DATE paper as Fig. 7(a) (Sec. V-C) since 2026-09-13. |
| `fig_medium` | `scripts/figures/plot_medium.py` → `fig_medium.png`: long-link rate sweep at the copper latency pad from `medium_ablation.csv` (batches 12 and 13; quotable rows only), iteration normalized to the glass design point per EP at 384 / 200 / 100 / 50 GB/s, EP 16/32/64; 100 GB/s marked as the copper-feasible point, 200 is a rate point, not copper; the pad-only control (glass 384 GB/s, 400 ns pad) drawn as a hollow marker at 384. Hop classification is rate-invariant, so the sweep moves makespan only and no energy curve accompanies it. In the DATE paper as Fig. 7 (Sec. V-D) since 2026-09-12, drawn on a 3.45x1.35 in canvas at full column width. |
| `fig_thermal_transient` | `scripts/figures/plot_thermal_transient.py` → `fig_thermal_transient.png`: `experiments/results/thermal/tile_schedule.csv` (status final, glass; delta_pp_K vs period_s per p_lo_W) and `tile_transient.csv` glass_stack_h200k_long step asymptote; period sweep landed 2026-09-08 (ef7a140: P_lo 210 W at 3 ms-3 s, P_lo 0 at 1 ms-3 s, coolant-40 controls identical); a first-order reference (tau = rise/2.197 from the step) is drawn dotted for the 210 W series and matches within 1% above 86 ms, overestimating below 30 ms |
...
| `fig_phases` | `scripts/figures/plot_a2a_share.py` → `fig_phases.png`: per model, expert_a2a_ms and compute_ms of the walk (`decomp_critpath.csv`) at the quotable hgx8_pkt `cliff_all.csv` row (mb 8); compute split by op class in proportion to the graph's longest compute path (`experiments/taskgraphs/*.txt`, sha-listed there; sub-class B, no producer) |
| `fig_panel`, `fig_coupling`, `fig_wgcross`, `fig_epplace` | schematic drawings, no simulation input |
| `fig_thermal_panel` | `scripts/figures/plot_thermal_panel.py --plane panel_plane_glass_cal_h200k_tin40.csv --plane-si panel_plane_si_cal_h200k_tin40.csv --tiles panel_cal_tiles.csv --variant cal_h200k_tin40 --variant-si cal_h200k_tin40_si` → `thermal_panel_steady.png` | `experiments/results/thermal/panel_plane_{glass,si}_cal_h200k_tin40.csv`, `panel_cal_tiles.csv` (5fd5190) ← `thermal_panel_map_body.inp` via `run_panel_extra.sh` (per-tile PIC 14/36/58 W at 1.15 pJ/bit, h=200000, Tin=40 → the calibrated point at which the die meets 85–90 °C) | committed chain. Glass PIC 87.3 °C outlet, Si 85.7; PIC−die ≤0.13 K. The 70k/55 field (135.1) is quoted in text as the pessimistic bound, not drawn. Sensitivities: `panel_map_tiles.csv`, `panel_h70k_tiles.csv`, `panel_extra_tiles.csv`, `panel_fhot_tiles.csv`, `tile_transient.csv`. |
...
**Energy charges changed 2026-09-13.** Every Glass-FB energy figure and table (`fig_energy`, `fig_medium` (b), `energy_table.tex`) now takes its per-tier pJ/bit from `scripts/figures/energy_consts.py`: electrical RDL tier 0.60-1.15 pJ/b (UCIe PHY to XSR SerDes PHY) and optical tiers 1.15-2.62 pJ/b (32-wavelength photonic link; 2.62 is a margin), all from Hsueh et al. Table 1 at 32 Gb/s per lane. This replaces the producer convention in `scripts/tier_energy.py` (electrical = optical bracket + 1.0 pJ/b unsourced RDL adder). The `link_J_iter_*` columns of `power_tiers.csv` for `glassfb*` therefore do NOT match the paper and are not read by any plotting script; hop-bytes and static columns are unchanged and still used. NVSwitch fabrics are unchanged.

**Static term corrected 2026-09-13 (later the same day).** The glass static term is no longer the `static_J_iter` / `static_J_iter_200G` columns of `power_tiers.csv` (5.3 / 9.12 W per panel), which applied ONE GPU's 960-wavelength laser-and-tuning budget to a whole 16-GPU panel (an 8.1x understatement against the 7,808 lit carriers). `energy_consts.py` now charges 42.9 W per panel for the 7,808 carriers the 4x4 panel modulates at the 200G/lane design point ((24 x 3 x 2 + 16 x 6.25) waveguides x 32): 1 mW per carrier behind a 25% wall-plug laser (31.2 W) plus two 0.75 mW rings per carrier (11.7 W). Those two static columns are therefore also unread by every paper figure.

**Single-value charges 2026-09-13 (user decision).** No favorable/conservative brackets remain in any paper energy figure or table: every tier has one value in `energy_consts.py` (electrical RDL 1.15 and optical 1.15 pJ/b from Hsueh Table 1; NVLink 5.0 and NIC 16 pJ/b from HotI'25; copper long links 16 pJ/b; NVSwitch static 7.5 W/GPU NVL72, 12.5 W/GPU HGX-8; glass static 42.9 W/panel). The NVSwitch rows are now also computed from bytes and these constants; the `link_J_iter_*` and `static_J_iter_*` columns of `power_tiers_pkt.csv` are unread.
...
**Fig. 4 side-by-side 2026-09-18.** `plot_thermal_panel.py` 1.68x1.62 in and `plot_thermal_transient.py` 1.82x1.5 in, drawn at the size they print at when Fig. 4(a)/(b) sit side by side in one column (0.46 / 0.52 linewidth). Panel title shortened, coolant label on one line, transient x ticks "1 ms / 10 / 100 / 1 s", right label "ring drift (nm)", legend at lower right. Fig. 4(a) MUST be generated with the h=200k arguments recorded in the row above, not the script defaults (those are h=70k and peak 135 C).
