...
| `fig_load` | `scripts/figures/plot_paper.py load` → `fig_load.png`: (a) EP=16 mb 4/8/16/32 quotable rows of `cliff_all.csv` (glassfb_800, nvl64_pkt_s1, glassfb) split by `decomp_critpath.csv`; (b) walks g64b800 and s1_16, (c) walk g128b800, q in multiples of 533 (glass 800) / 600 (NVL72) packets, rtos annotated |
...
| `fig_dse` | `scripts/figures/plot_paper.py dse` → `fig_dse.png`: every rung of the 8x8 flattened-butterfly (glassfb_8x8, opt_bw 128) and 8x8 electrical-mesh (glassfb_mesh8x8, GLASS_MAXDIST=1) walks from `cliff_postfix.csv` (batch 10, binary 95c35a7), x = q/533 (800 GB/s port BDP), with the 4x4 glassfb_800 quoted row as the reference line; points above the panel's range are clipped and labelled; EP=128 pending |
